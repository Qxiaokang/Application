...
class Util {
    ...
    public static TimeObject getYear(Calendar c, String formatString) {
        ...
        String tmp=String.format(formatString, c,c);
        c.set(year, 11, 31, 23, 59, 59);
        c.set(Calendar.MILLISECOND, 999);
        long endTime = c.getTimeInMillis();
        return new TimeObject(tmp, startTime, endTime);
    }
    
    public static TimeObject getYears(Calendar c, String formatString) {
        int year = c.get(Calendar.YEAR);
        // set calendar to first millisecond of the year
        c.set(year, 0, 1, 0, 0, 0);
        c.set(Calendar.MILLISECOND, 0);
        long startTime = c.getTimeInMillis();
        // set calendar to last millisecond of the year
        String tmp=String.format("%1$02d",c.get(Calendar.YEAR)-1900);
        c.set(year, 11, 31, 23, 59, 59);
        c.set(Calendar.MILLISECOND, 999);
        long endTime = c.getTimeInMillis();
        return new TimeObject(tmp, startTime, endTime);
    ...
    public static TimeObject getMonth(Calendar c, String formatString) {
        ...
        String tmp;
        if(formatString.equals("%d") ||formatString.equals("%02d"))
        	tmp=String.format(formatString, c.get(Calendar.MONTH));
        else
        	tmp=String.format(formatString, c,c);
